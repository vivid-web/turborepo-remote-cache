...
import { attachUsersToTeam } from "../server-fns/attach-users-to-team";
import { checkIfSomeUsersAreAttachedToTeam } from "../server-fns/check-if-some-users-are-attached-to-team";
...
function AttachUsersToTeamDialog({
	children,
	teamId,
	...
	const form = useAppForm({
		...
		validators: {
			...
			onSubmitAsync: async ({ value }) => {
				if (await checkIfSomeUsersAreAttachedToTeam({ data: value })) {
					return {
						fields: {
							userIds: {
								message:
		...
		onSubmit: async ({ value, formApi }) => {
			await attachUsersToTeam({ data: value });
		...
		<Dialog onOpenChange={setIsOpen} open={isOpen}>
			...
			<DialogContent>
				...
				<form
					...
					id={ATTACH_USERS_TO_TEAM_FORM_ID}
				>
					<form.AppField
						name="teamId"
						children={(field) => (
					...
					<form.Subscribe
						...
						children={([canSubmit, isSubmitting]) => (
							<ButtonWithPendingState
								...
								type="submit"
								form={ATTACH_USERS_TO_TEAM_FORM_ID}
								disabled={!canSubmit}
							>
								Save
							</ButtonWithPendingState>
...
AttachUsersToTeamDialog.queryOptions = attachableUserOptionsForTeamQueryOptions;

export { AttachUsersToTeamDialog };
